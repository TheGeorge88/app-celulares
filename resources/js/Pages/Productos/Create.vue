...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    const response = await fetch(route('api.productos.store'), {
      method: 'POST',
      headers: {
      ...
      <UForm :schema="schema" :state="state" class="space-y-6 max-w-2xl" @submit="onSubmit">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <UFormField label="Codigo" name="codigo">
            <UInput v-model="state.codigo" placeholder="PROD-001" />
          </UFormField>

          <UFormField label="Nombre" name="nombre">
            <UInput v-model="state.nombre" placeholder="Nombre del producto" />
          </UFormField>
        ...
        <UFormField label="Descripcion" name="descripcion">
          <UTextarea v-model="state.descripcion" placeholder="Descripcion del producto..." />
        </UFormField>
